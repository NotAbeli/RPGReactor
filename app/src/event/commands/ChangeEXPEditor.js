/**
 * ChangeEXPEditor - Editor for Change EXP event command (code 315)
 */
class ChangeEXPEditor {
    constructor(databaseManager, projectController) {
        ...
        // Parameters: [actorSelect, actorId, operation, operandType, operand, showLevelUp]
    ...
    show(command, callback) {
        ...
        if (command && command.code === 315) {
            ...
            this.showLevelUp = params[5] || false;
        } else {
            this.actorSelect = 0;
            this.actorId = 1;
            this.operation = 0;
    ...
    createModal() {
        ...
        this.modal.className = 'change-exp-editor-modal';
        ...
        container.className = 'change-exp-container';
    ...
    renderContent() {
        ...
        const container = this.modal.querySelector('.change-exp-container');
        container.innerHTML = '';
        ...
        `;
        header.innerHTML = `
            <h3 style="margin: 0; color: var(--color-text-strong); font-size: 16px;">${tt('Change EXP')}</h3>
        ...
        // Show Level Up checkbox
        const levelUpRow = document.createElement('div');
        levelUpRow.style.cssText = 'display: flex; align-items: center; gap: 8px; padding-top: 8px; border-top: 1px solid var(--color-border);';

        const levelUpCheckbox = document.createElement('input');
        levelUpCheckbox.type = 'checkbox';
        levelUpCheckbox.id = 'show-level-up-315';
        levelUpCheckbox.checked = this.showLevelUp;
        levelUpCheckbox.addEventListener('change', (e) => {
            this.showLevelUp = e.target.checked;
        });

        const levelUpLabel = document.createElement('label');
        levelUpLabel.htmlFor = 'show-level-up-315';
        ...
        levelUpRow.appendChild(levelUpLabel);
        content.appendChild(levelUpRow);
    ...
    createActorSelector() {
        ...
        fixedRadio.type = 'radio'; fixedRadio.name = 'actor-select-315'; fixedRadio.id = 'actor-fixed-315';
        fixedRadio.checked = (this.actorSelect === 0);
        fixedRadio.addEventListener('change', () => { this.actorSelect = 0; this.renderContent(); });
        const fixedLabel = document.createElement('label');
        fixedLabel.htmlFor = 'actor-fixed-315'; fixedLabel.textContent = tt('Fixed');
        ...
        varRadio.type = 'radio'; varRadio.name = 'actor-select-315'; varRadio.id = 'actor-variable-315';
        varRadio.checked = (this.actorSelect === 1);
        varRadio.addEventListener('change', () => { this.actorSelect = 1; this.renderContent(); });
        const varLabel = document.createElement('label');
        varLabel.htmlFor = 'actor-variable-315'; varLabel.textContent = tt('Variable');
    ...
    createOperationSection() {
        ...
        increaseRadio.name = 'operation-315';
        increaseRadio.id = 'increase-315';
        ...
        increaseLabel.htmlFor = 'increase-315';
        ...
        decreaseRadio.name = 'operation-315';
        decreaseRadio.id = 'decrease-315';
        ...
        decreaseLabel.htmlFor = 'decrease-315';
    ...
    createOperandSection() {
        ...
        constRadio.name = 'operand-type-315';
        constRadio.id = 'constant-315';
        ...
        constLabel.htmlFor = 'constant-315';
    ...
    buildCommand() {
        return {
            code: 315,
            indent: 0,
            parameters: [this.actorSelect, this.actorId, this.operation, this.operandType, this.operand, this.showLevelUp]
        };
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = ChangeEXPEditor;
}
