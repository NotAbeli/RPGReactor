...
## No-Battle (S12) — стандартный бой ампутирован

Проект — экшен-RPG (бой через SuperDuperBattle), Scene_Battle недостижим и вырезан: **рантайм** — модуль `RR.installNoBattle` в мосте: 301/enemy-ops (331–337/339/340/342) → no-op, `updateEncounter`/`encounterProgressValue` отключены, `isBattleTest`=false, `BattleManager.setup/startBattle` — warn-заглушки; боевые UI-классы (`Scene_Battle`, `Spriteset_Battle`, `Window_Battle*`, `Window_Party/ActorCommand`) затираются на **первом `Scene_Map.start`** (не в setup! — async=false скрипты плагинов исполняются после возврата setup, а HUDMaker/Camera алиасят Scene_Battle при загрузке). `Game_*` (Battler/Actor/Troop) НЕ тронуты — JsonEx сейвов требует живых конструкторов. **Редактор**: вкладки Skills/Classes/Enemies(MV)/Troops/States убраны из навигации, `BattleTestConfigModal` удалён, `PlaytestManager.battleTest` удалён, из палитры — 301/318/321/331–337/339/340/342, из ConditionalBranch — Class/Skill-условия. MV-редакторы скрытых вкладок живы (инстанцируются, недостижимы). **Готча**: типы вкладок БД обязаны быть уникальны — S10-коллизия `enemies` делала стандартный редактор врагов недостижимым (теперь у ИИ-врагов тип `enemyAI`).
