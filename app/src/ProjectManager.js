...
`;
    ...
    static MV_CATALOG_LOADER_SNIPPET = [
        ...
        '                SuperDuperSteps: "steps",',
        '                SuperDuperVariables: "variables",',
        '                SuperDuperDrop: "drop",',
        '                SuperDuperNotification: "notification",',
        '                SuperDuperSave: "save",',
        '                MOG_TitlePictureCom: "title",',
        '                SuperDuperSplash: "splash",',
        '                SuperDuperGameOver: "gameover",',
        '                SuperDuperMessage: "message",',
        '                SuperDuperChoices: "choices",',
        '                SuperDuperSettings: "settings",',
        ...
        '                // P33: реализация кодом прямо в нативе — не зависим от',
        '                // плагин-команды WaitAsync. Поля интерпретатора — тот же',
        '                // контракт, что и плагин (updateWaitMode/canMove-фикс P32',
        '                // читают _waitMode "waitAsync").',
        '                this._waitAsyncFrames = Math.max(0, Number(params[0] || 0));',
        ...
        '            };',
        '',
        '            // P39: СИСТЕМНАЯ АТАКА ОРУЖИЕМ — вся логика боя из карточки',
        ...
        '                    // P39: switch 76 = перезарядка. Старый CE#12 сам завершал',
        '                    // перезарядку (CE47) и продолжал атаку. НЕ блокируем.',
        '                    if ($gameVariables.value(69) > 0) return false;',
        '                    var cost = Math.max(0, Number(weapon.staminaCost) || 0);',
        '                    if (cost > 0 && $gameVariables.value(15) < cost) return false;',
        '                    if (cost > 0) {',
        '                        var st = $gameVariables.value(15);',
        '                        $gameVariables.setValue(15, Math.max(0, st - cost));',
        '                    }',
        '                    var cd = Math.max(0, Number(weapon.cooldown) || 0);',
        '                    if (cd > 0) $gameVariables.setValue(69, cd);',
        '                    $gameSwitches.setValue(2, true);',
        '                    var sw = Math.max(0, Number(weapon.swingSwitch) || 0);',
        '                    if (sw > 0) $gameSwitches.setValue(sw, true);',
        '                    if (weapon.dashName && String(weapon.dashName).trim() !== "" &&',
        '                        $gamePlayer && $gamePlayer.tryDash) {',
        '                        $gamePlayer.tryDash(String(weapon.dashName).trim());',
        '                    }',
        '                    if (typeof $gameSystem === "undefined" || !$gameSystem) return false;',
        '                    $gameSystem._rrAttack = {',
        '                        frames: Math.max(0, Number(weapon.windup) || 0),',
        '                        attackId: Math.max(1, Number(weapon.attackId) || 1),',
        '                        attackType: weapon.type === "ranged" ? "projectile" : "melee",',
        '                        attackSe: String(weapon.attackSe || "").trim(),',
        '                        swingSwitch: sw,',
        '                        fireRadius: Math.max(0, Number(weapon.fireRadius) || 0),',
        '                        fireColor: String(weapon.fireColor || "#e97451"),',
        '                        fireFrames: Math.max(0, Number(weapon.fireFrames) || 0)',
        '                    };',
        '                    return true;',
        '                } catch (e) {',
        '                    console.error("[RR] executeWeaponAttack:", e);',
        '                    return false;',
        '                }',
        '            };',
        '            p.command760 = function(params) {',
        '                RR.executeWeaponAttack();',
        '                return true;',
        '            };',
        '',
        '            // P39: отложенная атака — lazy: ставим алиас только когда',
        '            // Game_Map существует (vm-тесты могут не иметь его)',
        '            var _rrAtkInstalled = false;',
        '            var _rrAtkInstall = function() {',
        '                if (_rrAtkInstalled) return;',
        '                if (typeof Game_Map === "undefined" || !Game_Map.prototype || !Game_Map.prototype.update) return;',
        '                _rrAtkInstalled = true;',
        '                var _rrAtk_GMU = Game_Map.prototype.update;',
        '                Game_Map.prototype.update = function(isActive) {',
        '                    _rrAtk_GMU.call(this, isActive);',
        '                    try {',
        '                        var atk = $gameSystem && $gameSystem._rrAttack;',
        '                        if (!atk) return;',
        '                        if (atk.frames > 0) { atk.frames--; return; }',
        '                        if (atk.attackType === "projectile" && $gamePlayer.performProjectile) {',
        '                            $gamePlayer.performProjectile(atk.attackId);',
        '                        } else if ($gamePlayer.performMelee) {',
        '                            $gamePlayer.performMelee(atk.attackId);',
        '                        }',
        '                        if (atk.attackSe && typeof AudioManager !== "undefined") {',
        '                            try { AudioManager.playSe({ name: atk.attackSe, volume: 100, pitch: 100, pan: 0 }); } catch (e2) {}',
        '                        }',
        '                        if (atk.fireRadius > 0) {',
        '                            var gi = Game_Interpreter.prototype;',
        '                            if (gi.command700) gi.command700.call(',
        '                                { _params: [1, atk.fireRadius, 0, 0, atk.fireColor, "", ""] },',
        '                                [1, atk.fireRadius, 0, 0, atk.fireColor, "", ""]',
        '                            );',
        '                        }',
        '                        if (atk.swingSwitch > 0) {',
        '                            $gameSwitches.setValue(atk.swingSwitch, false);',
        '                        }',
        '                        $gameSystem._rrAttack = null;',
        '                        if (atk.fireRadius > 0) {',
        '                            $gameSystem._rrFireOff = { frames: atk.fireFrames || 6, color: atk.fireColor };',
        '                        }',
        '                    } catch (e) { console.error("[RR] attack exec:", e); }',
        '                };',
        '            };',
        '            _rrAtkInstall();',
        '            if (typeof Scene_Boot !== "undefined" && Scene_Boot.prototype) {',
        '                var _rrAtk_Boot_start = Scene_Boot.prototype.start;',
        '                Scene_Boot.prototype.start = function() {',
        '                    _rrAtk_Boot_start.call(this);',
        '                    _rrAtkInstall();',
        '                };',
        '            }',
        '',
        ...
        '            // P39: экспорт RR на window — натив 760 (executeWeaponAttack)',
        '            // доступен из внешних вызовов (CE, скрипты, дебаг)',
        '            if (typeof window !== "undefined") window.RR = RR;',
        '        },',
