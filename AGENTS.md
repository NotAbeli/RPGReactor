...
БД «Движок Agonia» (`data/AgoniaEngine.json`, редактор в `DatabaseAgoniaEditor`): секции `spriter` → SuperDuperSpriter (редактор — отдельная вкладка «Спрайтер» в `DatabaseSpriterEditor`, под «Героями»: скины/позы/NPC с условиями, грид-выбором, живым canvas-плеером; коллекции хранятся в MV-формате строк — `normalizeAgoniaValue` пропускает массивы JSON-объект-строк verbatim, `loadAgonia` дочернеет недостающие секции сидом из engineModules), `audio` → Аудио-студия (`AudioStudio.js`, окно из тулбара: библиотека bgm/bgs/me/se с волной и транспортом, микшер 6 каналов + правила автозапуска карт `Map Rules`, редактор эмбиент-зон `AudioStudioZones` — правит реальные `<aex>`-события карт; `Map Rules`/`Recent Tracks` — JSON-строки; reseed переносит studio-данные из существующего файла, а не дефолтит), `stamina` → SuperDuperMovement, `lighting` → SDLight, `camera` → SuperDuperCamera, `inventory` → SuperDuperInventory, `screen` → SuperDuperScreen (система). Сид: engineModules → манифест → дефолты (`agoniaSeedValues`); normalizeDataSystem добивает MZ-блоки MV-проектам (advanced/titleCommandWindow + ранний resize — MV-splash-плагины не зовут Scene_Boot.start).
