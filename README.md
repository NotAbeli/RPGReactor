...
- **Long events fold to their structure**: Conditional Branch, Show Choices, and Loop collapse from the arrow beside them, showing how many lines are hidden. Nested blocks keep their own state, folds survive edits above them, and each page remembers its folds across restarts.
- **Resizing a map can't silently destroy work**: shrinking now names the events it would delete and counts the tiles it would discard, and changes nothing until you confirm. A new anchor picker lets a map grow from the top or left, moving tiles and events together and offering to fix Transfer Player commands elsewhere that point at it.
- **Seven more languages are actually translated**: Chinese (Traditional and Simplified), Russian, Portuguese, German, French, and Greek were falling back to English for about a third of the interface, including the entire Effekseer Forge. The build now fails when a key ships untranslated.
- **The victory screen stops stuttering**: bitmap drawing was re-uploading the whole image to the graphics card on every operation, and text colours were re-read from the windowskin pixel by pixel. Uploads are batched and colours cached.
- **MV games behave like MV**: animations on party members are no longer mirrored, a choice from a plugin-created message window no longer crashes, and Set Movement Route with Wait no longer finishes a later event's route instantly.
- **The editor holds up under real use**: relaunching the packaged editor opens a second instance, toolbar Delete matches keyboard Delete, a map whose file can't be written no longer lingers in the tree, and the Web editor regained keyboard shortcuts, working menus, and honest save-failure reporting.
