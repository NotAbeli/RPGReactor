/**
 * ChangeHPEditor - Editor for Change HP event command (code 311)
 */
class ChangeHPEditor {
    constructor(databaseManager, projectController) {
        ...
        // Parameters: [actorSelect, actorId, operation, operandType, operand, allowDeath]
    ...
    show(command, callback) {
        ...
        if (command && command.code === 311) {
            ...
            this.allowDeath = params[5] || false;
        } else {
            this.actorSelect = 0;
            this.actorId = 1;
            this.operation = 0;
    ...
    createModal() {
        ...
        this.modal.className = 'change-hp-editor-modal';
        ...
        container.className = 'change-hp-container';
    ...
    renderContent() {
        ...
        const container = this.modal.querySelector('.change-hp-container');
        container.innerHTML = '';
        ...
        `;
        header.innerHTML = `
            <h3 style="margin: 0; color: var(--color-text-strong); font-size: 16px;">${tt('Change HP')}</h3>
        ...
        // Allow Death checkbox
        const deathRow = document.createElement('div');
        deathRow.style.cssText = 'display: flex; align-items: center; gap: 8px; padding-top: 8px; border-top: 1px solid var(--color-border);';

        const deathCheckbox = document.createElement('input');
        deathCheckbox.type = 'checkbox';
        deathCheckbox.id = 'allow-death-311';
        deathCheckbox.checked = this.allowDeath;
        deathCheckbox.addEventListener('change', (e) => {
            this.allowDeath = e.target.checked;
        });
    ...
    createActorSelector() {
        ...
        fixedRadio.type = 'radio'; fixedRadio.name = 'actor-select-311'; fixedRadio.id = 'actor-fixed-311';
        fixedRadio.checked = (this.actorSelect === 0);
        fixedRadio.addEventListener('change', () => { this.actorSelect = 0; this.renderContent(); });
        const fixedLabel = document.createElement('label');
        fixedLabel.htmlFor = 'actor-fixed-311'; fixedLabel.textContent = tt('Fixed');
        ...
        varRadio.type = 'radio'; varRadio.name = 'actor-select-311'; varRadio.id = 'actor-variable-311';
        varRadio.checked = (this.actorSelect === 1);
        varRadio.addEventListener('change', () => { this.actorSelect = 1; this.renderContent(); });
        const varLabel = document.createElement('label');
        varLabel.htmlFor = 'actor-variable-311'; varLabel.textContent = tt('Variable');
    ...
    createOperationSection() {
        ...
        increaseRadio.name = 'operation-311';
        increaseRadio.id = 'increase-311';
        ...
        increaseLabel.htmlFor = 'increase-311';
        ...
        decreaseRadio.name = 'operation-311';
        decreaseRadio.id = 'decrease-311';
        ...
        decreaseLabel.htmlFor = 'decrease-311';
    ...
    createOperandSection() {
        ...
        constRadio.name = 'operand-type-311';
        constRadio.id = 'constant-311';
        ...
        constLabel.htmlFor = 'constant-311';
    ...
    buildCommand() {
        return {
            code: 311,
            indent: 0,
            parameters: [this.actorSelect, this.actorId, this.operation, this.operandType, this.operand, this.allowDeath]
        };
    }
