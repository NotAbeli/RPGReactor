...
`;
    ...
    static MV_CATALOG_LOADER_SNIPPET = [
        ...
        '            };',
        '            var active = cfg["Enabled on Startup"] !== false;',
        '            var scrW = Number(cfg["Screen Width"]) || 1280;',
        '            var scrH = Number(cfg["Screen Height"]) || 720;',
        '',
        '            var fragmentSrc = [',
        ...
        '            };',
        '            RRScreenFilter.prototype.applySettings = function(s) {',
        '                var u = this.uniforms;',
        '                u.uIntensity = s.intensity;',
        '                u.uBlur = s.blur;',
        '                u.uBloom = s.bloom;',
        '                u.uBloomThresh = s.bloomthresh;',
        '                u.uColorTemp = s.colortemp;',
        '                u.uSharpen = s.sharpen;',
        '                u.uWave = s.wave;',
        '                u.uChroma = s.chroma;',
        '                u.uScanline = s.scanline;',
        '                u.uNoise = s.noise;',
        '                u.uSaturation = s.saturation;',
        '                u.uContrast = s.contrast;',
        '                u.uBrightness = s.brightness;',
        '            };',
        '',
        '            var filter = new RRScreenFilter();',
        '            var presets = {};',
        '            settings.active = active;',
        '',
        '            // $gameSystem._superDuperConfig compatibility: SuperDuperSplash',
        '            // and SuperDuperGameOver drive their CRT-phase effects through',
        '            // this block and the SUPERDUPER plugin command channel.',
        '            if (typeof Game_System !== "undefined" && Game_System.prototype) {',
        '                var rrGsInit = Game_System.prototype.initialize;',
        '                Game_System.prototype.initialize = function() {',
        '                    rrGsInit.call(this);',
        '                    var base = JSON.parse(JSON.stringify(settings));',
        '                    this._superDuperConfig = base;',
        '                    this._superDuperTarget = JSON.parse(JSON.stringify(base));',
        '                    this._superDuperFrames = {};',
        '                    this._superDuperSavedPresets = {};',
        '                };',
        '            }',
        '',
        '            var applyToScene = function(scene, on) {',
        '                if (!scene) return;',
        '                var cur = scene.filters || [];',
        ...
        '                    var next2 = cur.slice();',
        '                    next2.splice(idx, 1);',
        '                    scene.filters = next2.length > 0 ? next2 : null;',
        '                }',
        '            };',
        '',
        '            RR.screen.crt = {',
        ...
        '                        $gameSystem._superDuperConfig.active = this.active;',
        '                    }',
        '                    if (typeof SceneManager !== "undefined" && SceneManager._scene) {',
        '                        applyToScene(SceneManager._scene, this.active);',
        '                    }',
        '                },',
        '                applyPreset: function(name) {',
        '                    var key = String(name || "").toLowerCase();',
        '                    var saved = null;',
        '                    if (typeof $gameSystem !== "undefined" && $gameSystem && $gameSystem._superDuperSavedPresets) {',
        '                        saved = $gameSystem._superDuperSavedPresets[key];',
        '                    }',
        '                    if (saved) {',
        '                        var applied = JSON.parse(JSON.stringify(settings));',
        '                        for (var k in saved) {',
        '                            if (saved.hasOwnProperty(k) && k in applied) applied[k] = saved[k];',
        '                        }',
        '                        filter.applySettings(applied);',
        '                        return true;',
        '                    }',
        '                    var p = presets[key];',
        '                    if (!p) return false;',
        '                    filter.applySettings(p);',
        '                    return true;',
        '                },',
        '                registerPreset: function(name, s) { presets[String(name).toLowerCase()] = s; },',
        ...
        '                    if (action === "ON") { RR.screen.crt.setActive(true); return; }',
        '                    if (action === "OFF") { RR.screen.crt.setActive(false); return; }',
        '                    if (!gs) return;',
        '                    var config = gs._superDuperConfig;',
        '                    var target = gs._superDuperTarget;',
        '                    var frames = gs._superDuperFrames;',
        '                    if (action === "SET") {',
        '                        var param = String(args[1] || "").toLowerCase();',
        '                        var value = Number(args[2]);',
        '                        var duration = Number(args[3]) || 0;',
        '                        if (!isNaN(value) && config.hasOwnProperty(param)) {',
        '                            if (duration > 0) {',
        '                                target[param] = value;',
        '                                frames[param] = duration;',
        '                            } else {',
        '                                config[param] = value;',
        '                                target[param] = value;',
        '                                frames[param] = 0;',
        ...
        '                        var tp = gs._superDuperSavedPresets && gs._superDuperSavedPresets[pName];',
        '                        if (tp) {',
        '                            var pd = Number(args[2]) || 0;',
        '                            for (var pk in tp) {',
        '                                if (tp.hasOwnProperty(pk) && config.hasOwnProperty(pk)) {',
        '                                    if (pd > 0) { target[pk] = tp[pk]; frames[pk] = pd; }',
        '                                    else { config[pk] = tp[pk]; target[pk] = tp[pk]; }',
        '                                }',
        '                            }',
        '                        }',
        '                    } else if (action === "SAVE_PRESET") {',
        '                        var sName = String(args[1] || "").toLowerCase();',
        '                        if (sName) {',
        '                            if (!gs._superDuperSavedPresets) gs._superDuperSavedPresets = {};',
        '                            gs._superDuperSavedPresets[sName] = JSON.parse(JSON.stringify(config));',
        '                        }',
        '                    }',
        '                }',
        '            };',
        '',
        '            // SUPERDUPER plugin command channel (Splash/GameOver use it).',
        '            if (typeof Game_Interpreter !== "undefined" && Game_Interpreter.prototype) {',
        '                var rrPc = Game_Interpreter.prototype.pluginCommand;',
        '                Game_Interpreter.prototype.pluginCommand = function(command, args) {',
        '                    if (command && String(command).toUpperCase() === "SUPERDUPER") {',
        '                        RR.screen.crt.handleCommand(args || []);',
        '                        return;',
        '                    }',
        '                    if (rrPc) return rrPc.apply(this, arguments);',
        '                };',
        '            }',
        '',
        ...
        '                applyToScene(this, RR.screen.crt.active);',
        '            };',
        '            var rrSbu = Scene_Base.prototype.update;',
        '            Scene_Base.prototype.update = function() {',
        '                rrSbu.apply(this, arguments);',
        '                var crt = RR.screen.crt;',
        '                var config = (typeof $gameSystem !== "undefined" && $gameSystem) ? $gameSystem._superDuperConfig : null;',
        '                var target = (typeof $gameSystem !== "undefined" && $gameSystem) ? $gameSystem._superDuperTarget : null;',
        '                var frames = (typeof $gameSystem !== "undefined" && $gameSystem) ? $gameSystem._superDuperFrames : null;',
        '                var changed = false;',
        '                if (config && target && frames) {',
        '                    for (var key in frames) {',
        '                        if (frames[key] > 0) {',
        '                            config[key] += (target[key] - config[key]) / frames[key];',
        '                            frames[key]--;',
        '                            if (frames[key] <= 0) config[key] = target[key];',
        '                            changed = true;',
        '                        }',
        '                    }',
        '                }',
        '                if (config && config.active !== crt.active) {',
        '                    crt.setActive(config.active);',
        '                }',
        '                if (changed && config) {',
        '                    filter.applySettings(config);',
        '                }',
        '                if (crt.active) {',
        '                    filter.uniforms.uWaveTime = (filter.uniforms.uWaveTime + 0.05) % (Math.PI * 2);',
        '                    filter.uniforms.uNoiseTime = (filter.uniforms.uNoiseTime + 0.01) % 1.0;',
        '                }',
        '            };',
        '        },',
        ...
        '                if (mult !== undefined && mult !== null && mult !== "") args.push(String(mult));',
        ...
        '                723, 724, 725, 726, 727, 729, 730, 731, 732, 733, 734, 735,',
