 ...
 * @author SumRndmDde (Исправлено: Korolev)
 *
 * @param Game Window
 * @default ====================================
 *
 ...
 *
 * [Супер-пупер монолитный фикс от Korolev]:
 * - Исправлена критическая ошибка "Maximum call stack size" при работе 
 * вместе с плагином SRD_SuperToolsEngine.
 * - Исправлена ошибка рендеринга PIXI, вызывавшая зазоры (щели) 
 * между тайлами событий и окружения при изменении разрешения.
 * - Добавлен СТАБИЛЬНЫЙ форсированный запуск полноэкранного режима (исправлен баг "через раз").
 *
 * ==============================================================================
...
Imported["SumRndmDde Game Upgrade"] = 1.35;
...
// [Фикс Korolev]: корректное числовое сравнение версии (строковое ломается на NW.js >= 0.100: "0.114.0" < "0.13.0")
(function(){
	var _nwVer = (process.versions['node-webkit'] || process.versions.nw || '0.0.0').split('.');
	var _nwMajor = parseInt(_nwVer[0], 10) || 0;
	var _nwMinor = parseInt(_nwVer[1], 10) || 0;
	var _nwOK = _nwMajor > 0 || (_nwMajor === 0 && _nwMinor >= 13);
	var _rpgParts = (Utils.RPGMAKER_VERSION || '0.0.0').split('.');
	var _rpgMajor = parseInt(_rpgParts[0], 10) || 0;
	var _rpgMinor = parseInt(_rpgParts[1], 10) || 0;
	var _rpgOK = _rpgMajor > 1 || (_rpgMajor === 1 && _rpgMinor >= 6);
	_.isNewNWjs = _nwOK && _rpgOK;
})();
...
// [Фикс Korolev]: Устранение зазоров и размытия тайлов
const _ShaderTilemap_update = ShaderTilemap.prototype.update;
ShaderTilemap.prototype.update = function() {
	_ShaderTilemap_update.apply(this, arguments);
	this.roundPixels = _.params['Round Pixels'];
};
...
// [Фикс Korolev]: Изменено имя переменной-алиаса для избежания конфликта
SRD.GameUpgrade_DataManager_isDatabaseLoaded = DataManager.isDatabaseLoaded;
DataManager.isDatabaseLoaded = function() {
	if(!SRD.GameUpgrade_DataManager_isDatabaseLoaded.apply(this, arguments)) return false;
...
// [Фикс Korolev]: Перенос логики фулскрина в Scene_Boot для стабильного запуска (без "через раз")
const _Scene_Boot_start = Scene_Boot.prototype.start;
Scene_Boot.prototype.start = function() {
	_Scene_Boot_start.apply(this, arguments);
	if (_.windowSettings['fullscreen']) {
		setTimeout(function() {
