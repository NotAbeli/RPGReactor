...
function fakeWindow({ worldX, worldY, originX = 0, originY = 0, innerWidth = 200, innerHeight = 264 }) {
    return {
        ...
        _clientArea: {
            ...
            worldTransform: {
                a: 1, b: 0, c: 0, d: 1, tx: worldX, ty: worldY,
                apply(point) { return { x: this.tx + point.x, y: this.ty + point.y }; }
            }
        }
    };
}
...
function asV8ReadsIt(filterArea, worldTransform) {
    return {
        x: worldTransform.a * filterArea.x + worldTransform.tx,
        y: worldTransform.d * filterArea.y + worldTransform.ty,
        width: worldTransform.a * filterArea.width,
        height: worldTransform.d * filterArea.height
    };
}
